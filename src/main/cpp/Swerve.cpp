 ...
 */
SwerveModule::SwerveModule(ctre::phoenix::motorcontrol::can::TalonFX *driveMotor_,
 ...
 * @param targetAngle The desired angle of the wheel in Degrees
 */
void SwerveModule::DriveSwerveModuleMeters(double driveSpeed, double targetAngle)
{
 ...
 */
SwerveDrive::SwerveDrive(ctre::phoenix::motorcontrol::can::TalonFX *_FLDriveMotor,
 ...
 */
double SwerveDrive::GetIMURadians()
{
 ...
 */
void SwerveDrive::ResetConeOdometry()
{
 ...
 */
void SwerveDrive::ResetConeOdometry(Pose2d position)
{
  wpi::array<SwerveModulePosition, 4> positions = {FLModule->GetSwerveModulePosition(),
                                                    FRModule->GetSwerveModulePosition(),
                                                    BLModule->GetSwerveModulePosition(),
 ...
 */
void SwerveDrive::ResetTagOdometry()
{
 ...
 */
void SwerveDrive::ResetTagOdometry(Pose2d position)
{
  wpi::array<SwerveModulePosition, 4> positions = {FLModule->GetSwerveModulePosition(),
                                                    FRModule->GetSwerveModulePosition(),
                                                    BLModule->GetSwerveModulePosition(),
 ...
 */
void SwerveDrive::UpdateOdometry(units::second_t currentTime)
{
  wpi::array<SwerveModulePosition, 4> positions = {FLModule->GetSwerveModulePosition(),
                                                    FRModule->GetSwerveModulePosition(),
                                                    BLModule->GetSwerveModulePosition(),
 ...
 * @param currentTime The current FPGA time of the robot.
 */
void SwerveDrive::UpdateConeOdometry()
{
  wpi::array<SwerveModulePosition, 4> positions = {FLModule->GetSwerveModulePosition(),
                                                    FRModule->GetSwerveModulePosition(),
                                                    BLModule->GetSwerveModulePosition(),
 ...
 * @param currentTime The current FPGA time of the robot.
 */
void SwerveDrive::UpdateTagOdometry()
{
  wpi::array<SwerveModulePosition, 4> positions = {FLModule->GetSwerveModulePosition(),
                                                    FRModule->GetSwerveModulePosition(),
                                                    BLModule->GetSwerveModulePosition(),
 ...
 */
void SwerveDrive::AddPositionEstimate(Translation2d poseEstimate, units::second_t timeOfEstimate)
{
 ...
 * @param timeOfEstimate The FPGA time of the robot when this measuremente was recorded
 */
void SwerveDrive::AddPositionEstimate(Pose2d poseEstimate, units::second_t timeOfEstimate)
{
  // wpi::array<double, 3> stdDevs = {10.0, 10.0, 25.0};
  // odometry->SetVisionMeasurementStdDevs(stdDevs);
...
Pose2d SwerveDrive::GetConeOdometryPose()
{
  Pose2d pose = coneOdometry->GetPose();
  return Pose2d(pose.Y(), pose.X(), pose.Rotation());
}

Pose2d SwerveDrive::GetTagOdometryPose()
{
 ...
 */
void SwerveDrive::SetModuleStates(std::array<SwerveModuleState, 4> states)
{
 ...
 */
bool SwerveDrive::DriveToPose(Pose2d target, double elapsedTime)
{
 ...
 * @param elapsedTime the time since our last iteration of the pid loop
 */
bool SwerveDrive::DriveToPoseTag(Pose2d target, double elapsedTime)
{
 ...
 * @param elapsedTime the time since our last iteration of the pid loop
 */
bool SwerveDrive::DriveToPoseConeOdometry(Pose2d target, double elapsedTime)
{
 ...
 */
bool SwerveDrive::DriveToPose(Pose2d current, Pose2d target, double elapsedTime,
 ...
 */
double SwerveDrive::TurnToPointDesiredSpin(Translation2d point, double elapsedTime, double allowableErrorRotation, double spinMaxSpeed, double spinMaxAccel, double spinP, double spinI)
{
...
void SwerveDrive::ResetTrajectoryList()
{
...
void SwerveDrive::SetAllianceColorRed()
{
  isRedAlliance = true;
}

void SwerveDrive::SetAllianceColorBlue()
{
  isRedAlliance = false;
}
 ...
 * Run in auton Init
 */
void SwerveDrive::InitializeTrajectory(string trajectoryString)
{
 ...
 * Run in auton Init
 */
void SwerveDrive::InitializeTrajectory(string trajectoryString, units::meters_per_second_t velocity, units::meters_per_second_squared_t acceleration)
{
 ...
 */
void SwerveDrive::SetNextTrajectory()
{
  BeginPIDLoop();
 ...
 */
bool SwerveDrive::FollowTrajectory(units::second_t time, double elapsedTime)
{
  ...
  if (trajectory.getTotalTime() < time && fabs(xDistance) < S_ALLOWABLE_ERROR_TRANSLATION && fabs(yDistance) < S_ALLOWABLE_ERROR_TRANSLATION && fabs(thetaDistance) < S_ALLOWABLE_ERROR_ROTATION)
  {
 ...
 */
bool SwerveDrive::TurnToPixel(double offset, double elapsedTime)
{
...
bool SwerveDrive::TurnToPixelCone(double offset, double elapsedTime)
{
  // Use PID  to determine our desired spin speed
 ...
 * @param elapsedTime time since function last called
 */
bool SwerveDrive::StrafeToPole(double offsetX, double offsetY, double xGoal, double yGoal, double elapsedTime)
{
...
*/
bool SwerveDrive::StrafeToPoleFast(double offsetX, double offsetY, double xGoal, double yGoal, double elapsedTime)
{
...
// SmartDashboard::PutNumber("Intended Vel", intendedVelocity);
  ...
  runningIntegralX += offsetX;
  intendedI = std::clamp(0.03 * runningIntegralX, -1 * P_STRAFE_KI_MAX, P_STRAFE_KI_MAX);
  intendedVelocity = std::clamp(0.4 * offsetX + intendedI, -1 * P_STRAFE_MAX_SPEED, P_STRAFE_MAX_SPEED);
  lastX += std::clamp(intendedVelocity - lastX, -1 * P_STRAFE_MAX_ACCEL * elapsedTime,
                      P_STRAFE_MAX_ACCEL * elapsedTime);
  runningIntegralX += offsetX;
...
*/
bool SwerveDrive::BalanceOnCharger(double elapsedTime){
  ...
  if (!waitingOnPlatform)
  {
    if (fabs(gyroRot) < 4) // if we are in a 4 degree dead zone, then we basically have no error
      gyroRot = 0;
    intendedYVel = std::clamp(gyroRot *  0.05, -0.225, 0.225); // Use simple P to figure out the speed you should go
...
*/
void SwerveDrive::StrafeLock()
{
  FLModule->DriveSwerveModulePercent(0, 90);
  FRModule->DriveSwerveModulePercent(0, 90);
